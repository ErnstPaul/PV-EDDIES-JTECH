%% Paul_OPT_Step3RETRO_TRACER.m (version 3.1)
%Author: Yves Morel
...
function [temperaturetraj,salinitytraj,densitytraj,PVtraj,vorttraj,Ritraj]...
    = Paul_OPT_Step3RETRO_TRACER(lontraj,lattraj,zedtraj,icel,jcel,kcel,day2use,month2use,year2use,dt,refProf,u4D,v4D)
...
pvx = (zetax2*rhox2-zetax1*rhox1)*0.1/dXDiff(i+1,j);
...
pvy = (zetay2*rhoy2-zetay1*rhoy1)*0.1/dYDiff(i,j+1);
...
rhoz2= 0.25*( Grho(i,j,k+1)+Grho(i+1,j,k+1)+...
    Grho(i,j+1,k+1)+Grho(i+1,j+1,k+1));
pvz = ((zetaz2+fc)*rhoz2-(zetaz1+fc)*rhoz1)*0.1/dZDiff(k+1);
...
Ritraj=-(g*dzrhopv/rho0*0.1)/(dzUpv^2+dzVpv^2);
end %ENDFUNCTION