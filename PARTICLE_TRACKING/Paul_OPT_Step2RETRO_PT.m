%% Paul_OPT_Step2RETRO_PT.m (version 2.1)
%Author: Yves Morel
...
function [lontraj,lattraj,zedtraj,utraj,vtraj,wtraj,temperaturetraj,salinitytraj,densitytraj,PVtraj,vorttraj,Ritraj,timetraj]...
    = Paul_OPT_Step2RETRO_PT(lontraj,lattraj,zedtraj,utraj,vtraj,wtraj,temperaturetraj,salinitytraj,densitytraj,PVtraj,vorttraj,Ritraj,ipart,Tinit,Tend,refProf)
...
length2use = length(Tend:Tinit)*length(1:Ntot);
timetraj = zeros(1,length2use);
timetraj(1)=0.5/Ntot;
for time = fliplr(Tend:Tinit) % loop for trajectory on days
    tic
    disp("Performing backwards calculations for particle " + num2str(ipart) + " on " + datestr(time))
    ...
    for dt=1:Ntot
        ...
        for i = 1:2
            for j = 1:2
                for t = 1:4
                    w(i,j,:,t) = (gradient(squeeze(u(i,j,:,t))) + gradient(squeeze(v(i,j,:,t)))).*-0.001;
                end
            end
        end
        ...
        dtsec=(Ntot+1-dt)*delt;%centered scheme, velocity calculated ar regular times
    ...
    = Paul_OPT_Step3RETRO_TRACER(lontraj(ntraj),lattraj(ntraj),zedtraj(ntraj),icel,jcel,kcel,day2use,month2use,year2use,dt,refProf,u4D,v4D);
