%% Paul_OPT_EdgeParams.m (version 3.1)
%Author: Paul Ernst
...
%--------------------------------------
%Purpose: Takes a request for comparing eddy tracking methods and does so (EDGE-PARAMS, FIG3)
...
for RPTLOOP = 1:RPT
    ...
    if optimizeAandB
        count = 1;
        masterAB = zeros(2,length(a)*length(b));
        optLen = length(a)*length(b);
        for i = 1:length(a)
            for j = 1:length(b)
                %This gets every pair as (A | B) =/= (B | A)
                masterAB(1,count) = a(1,i);
                masterAB(2,count) = b(1,j);
                count = count + 1;
            end
        end
    end
