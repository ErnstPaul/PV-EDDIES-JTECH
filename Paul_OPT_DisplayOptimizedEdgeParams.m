...
%--------------------------------------
%Purpose: Displays the optimized parameters of edge methods (Figure 3)
...
%% Inputs
titlestring = "OptimizedEdgeParams";
PlotSources = ["OPT_E_VORT_T_STD", "OPT_E_OW_T_STD"];
xLabels = ["STD Factor", "STD Factor"];
...
for nTile = 1:numPanels
    ...
    [MaxVal,MaxInd] = nanmax(mean(totalSims,1));

    %Plot individual lines
    plot(xToPlot,nanmean(totalNum,1),'--', 'linewidth', 7)
    hold on
    plot(xToPlot,nanmean(totalRatio,1),'--', 'linewidth', 7)
    plot(xToPlot,nanmean(totalSpatialC,1),'--', 'linewidth', 7)
    plot(xToPlot,nanmean(totalSpatialR,1), '--', 'linewidth', 7)
    plot(xToPlot,nanmean(totalSims,1)/100, 'linewidth', 11)
    plot([xToPlot(MaxInd) xToPlot(MaxInd)],[0 1], '--k', 'linewidth', 7)
    ...
    yTxT = 0.95;
    text(xTxT, yTxT, panelLabels(nTile), 'FontSize', 48)
