%% Paul_OPT_SSMOVIE_PV.m (version 2.2)
%Author: Paul Ernst
...
%Configure output
outdirinterim = ['/MOVIE/FRAMES/FINALMOVIE/' num2str(depthlevel) '/'];
...
for i=[1:length(list)]
    ...
    load(filename, 'PV_ISO', 'U_ISO','V_ISO','F_ISO', 'date_num', 'U', 'V')
    if exist('PV_ISO','var')==1
        PV_ISO = double(PV_ISO(:,:,depthlevel)-F_ISO);
    ...
    [hpv5, htv5] = m_vec(100, EL-diffY*0.15, NL+diffX*-0.1, 100, 0, 'w', 'key', '1 m s^{-1}',...
    ...
    for pls = 1:length(idlist)
        if ismember(date_num,Edd{4,pls})
            ...
            if ((Edd{2,pls}(indexmat) > WL) && (Edd{2,pls}(indexmat) < (EL-2))...
                ...
                m_text((Edd{2,pls}(indexmat)),(Edd{3,pls}(indexmat)),int2str(Edd{1,pls}), 'color', 'k', 'fontsize', 14)
            end
        end
    end
    ...
    for pls = 1:length(idlist2)
        if ismember(date_num,Edd2{4,pls})
            ...
            if ((Edd2{2,pls}(indexmat) > WL) && (Edd2{2,pls}(indexmat) < (EL-2))...
                ...
                m_text((Edd2{2,pls}(indexmat)),(Edd2{3,pls}(indexmat)),int2str(Edd2{1,pls}), 'color', 'k', 'fontsize', 14)
            end
        end
    end
    ...
    %NOTE NOTE NOTE NOTE NOTE NOTE: 68, 28 is the (lon, lat) that this
    ...
    set(gca,'fontsize',36);
    colorbar(gca,'Position',[0.732031250000002 0.113526570048309 0.0194901019588132 0.811594202898551]); colormap(redblue());
    %I don't know how this colorbar position varies with screen resolution. You
    %may need to fiddle with this.
    ylabel('Latitude');
    xlabel('Longitude');
    title(['PV_I_S_O Anomalies (s^-^1) and all IDs At Depth'], 'fontsize', 36);
    caxis([-1e-5 1e-5]);
    %This color axis is generally pretty good for VORTA. But, you can
