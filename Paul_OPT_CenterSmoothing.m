%% Paul_OPT_CenterSmoothing.m (version 3.1)
%Author: Paul Ernst
...
%PE 6/2/2022 - Adjusted to work for CenterParams
...
%--------------------------------------
%Purpose: Takes a request for comparing eddy tracking methods and does so (CENTER-SMTH, FIG2)
...
%--------------------------------------
%% Inputs
function Paul_OPT_CenterSmoothing(inputDir)
load('DefaultData_OPT.mat') %See Paul_OPT_CreateDefaultSettings.m
Paul_OPT_LoadDefaultSettings(); %See corresponding function

%Title for info mat
titlestrings = ["OPT_C_VORT_T_SMTH", "OPT_C_VORT_WA_SMTH", "OPT_C_OW_T_SMTH", "OPT_C_OW_WA_SMTH", "OPT_C_PV_ISO_SMTH"];
RPT = length(titlestrings);
%METHODS
centerMethodss = ["VORT_T","VORT_WA","OW_T","OW_WA","PV_ISO"]; %list of methods to find center: SSH, PV_ISO, VORT, MV, OW, VG, LNAM
edgeMethods = ["SSH"]; %list of methods to find edge: SSH, PV_ISO, VORT, MV, OW, VG

%ONLY USE FOR OPTIMIZING BOTH SMOOTH AND RATIO WITH MANY METHODS
for RPTLOOP = 1:RPT
    if (RPT > 1)
        titlestring = titlestrings(RPTLOOP);
        centerMethods = centerMethodss(RPTLOOP);
    end
    %ONLY USE FOR OPTIMIZING BOTH SMOOTH AND RATIO WITH MANY METHODS
    ...
    save(filename_out, "totalSims","totalRatio","totalSpatialC","totalSpatialR","totalNum",...
        "masterMethods","masterDirList","inputDir","datetimeNow","xToPlot","winner");
end %END RPTLOOP
